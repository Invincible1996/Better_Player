...
class _VideoProgressBarState
  ...
  Widget build(BuildContext context) {
    void seekToRelativePosition(Offset globalPosition) {
      ...
      // relative = relative <= 0.001 ? 0 : (relative >=1 ? 1 : relative);
      if (relative >= 0 && relative <= 1) {
        debugPrint('70---better_player_material_progress_bar-----${relative.toStringAsFixed(2)}');
        debugPrint('70---better_player_material_progress_bar-----${double.parse(relative.toStringAsFixed(2))}');
        final Duration position = controller.value.duration * double.parse(relative.toStringAsFixed(2));
